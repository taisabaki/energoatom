...
class PermissionsHandler extends Handler{
    ...
    public function save($params){
        ...
        foreach($data as $p => $array){
            ...
            $class = $array['class'] == "-1" ? 1 : $this->db->escape($array['class']);
...
// inner helper functions
    ...
    private function getContent($id, $year){
        ...
        $sql = "select  `permission_content`.`paragraph`,
                        `permission_content`.`content` as `content`,
                        DATE_FORMAT(`permission_content`.`term`,'%d.%m.%Y') as `term`,
                        `permission_content`.`executive` as `executive`,
                        `permission_content`.`report` as `report`,
                        `permission_content`.`section` as `section`,
                        `permission_content`.`class` as `class`,
                        `permission_content`.`status` as `status`,
                        `permission_content`.`state` as `state`,
                        `departments`.`is_active` as `exec_active`
                from `permission_content` join `permission` join `departments`
                on `permission_content`.`fk_to_permission` = `permission`.`id` and `permission_content`.`executive` = `departments`.`id`
    ...
    private function getDepartmentsList(){
        ...
        $sql = "select * from `departments` where `id` != 1 and `is_active` = 1 order by `name`";
        $result = $this->db->query($sql);
    ...
    private function getClassesList(){
        $sql = "select * from `class` where `is_active` = 1 and `id` != 0 order by `id`";
