    ...
    var methods = {
        init: function(){
            ...
            elements.create_button.on('click.pad.create', function(){
                ...
                if (!valid){
                    cursor_release();
//                    info('Ошибка!'.emphasize('danger'), 'Нельзя создать протокол. Подробнее: см. <a class = "output-anchor" href = "#protocol-log">журнал</a>.'.emphasize('danger'));
                    output_alert('Ошибка!', 'Нельзя создать протокол. Подробнее: см. <a class = "output-anchor" href = "#protocol-log">журнал</a>.');
        ...
        load: function(id, year){
            ...
            function load(){
                ...
                if($DATA.form_data.link === null){
                    form_elements.service.view_button.addClass('text-muted').attr('title', 'Сканированная версия проткоола не загружена').prop('disabled', true);
                } else {
                    form_elements.service.view_button1.add(form_elements.service.view_button2).data('link', $DATA.form_data.link);
                    form_elements.service.view_button.attr('title', 'Смотреть сканированную версию протокола').prop('disabled', false);
                }
    ...
    var methods = {
        ...
        init: function(){
            ...
            $(document).on('click.form.save', elements.service.save_button.selector, function(){
                ...
                output_confirm('Перенести в архив?', 'Данный протокол будет перенесен в архив.</br>Подтвердите действие.')
                    .then(function(data){
                        if(data){
                            $this.form('archive');
                        }
...
//                    info('Ошибка!'.emphasize('danger'), 'Нельзя сохранить данные. Подробнее: см. <a class = "output-anchor" href = "#protocol-log">журнал</a>.'.emphasize('danger'));
                    output_alert('Ошибка!', 'Нельзя сохранить данные. Подробнее: см. <a class = "output-anchor" href = "#protocol-log">журнал</a>.');
                    return false;
                }
...
//************************** end of fields validation
                ...
                $.request('save', request_data).then(function(){
                    ...
                    if($DATA.save){
                        msg = 'Данные успешно сохранены. Протокол' + ('[' + $DATA.units[current_protocol.id].unit_name + ' ППР-' + current_protocol.year + ']').emphasize('info');
...
(function($){
    ...
    var methods = {
        ...
        close: function(){
            ...
            this.hide();
            this.data('shown', false);
            db.paragraphmenu.data('triggered').removeClass('paragraph-triggered');
