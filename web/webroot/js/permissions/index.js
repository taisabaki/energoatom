...
$.extend(db,
    {
        pad: $('#create-permission-pad-wrap'),
        list: $('#permission-list-wrap'),
        form: $('#permission-form-wrap'),
        pad_toggle:  $('#create-permission-menu-toggle'),
        console: $('.console'),
...
db.paragraphs = function(param1, param2, param3){
    ...
    if (param1 == 'current') {
        ...
        if (param2 == 'content' || param2 == 'state' || param2 == 'term' || param2 == 'executive' || param2 == 'class' || param2 == 'section' || param2 == 'status' || param2 == 'report') return contents.filter('.active').find('.ajax-data' + param2.wrapToAttr('name'));
    ...
    var methods = {
        init: function(){
            ...
            this.data({
                ...
                shown: !this.is(':hidden'),
                elements: {
                    create_button: $('#permission-create'),
                    select_units: $('#permission-unit'),
                    select_years: $('#permission-year'),
                    select_date: $('#permission-date'),
                    select_license: $('#permission-license'),
            ...
            elements.create_button.on('click.pad.create', function(){
                ...
                var license = elements.select_license.val();
                var count = 0;
                var valid = true;

                var msg = 'Нельзя создать ОД: ';
                if(id == -1){
                    ...
                    msg += 'не заполнено поле' + '[Номер разрешения]'.emphasize('danger');
                    count++;
                    valid = false;
                ...
                if (!valid){
                    cursor_release();
//                    info('Ошибка!'.emphasize('danger'), 'Нельзя создать ОД. Подробнее: см. <a class = "output-anchor" href = "#permission-log">журнал</a>.'.emphasize('danger'));
                    output_alert('Ошибка!', 'Нельзя создать ОД. Подробнее: см. <a class = "output-anchor" href = "#permission-log">журнал</a>.');
                    db.console.console('log', consoleSentence(msg), 0);
                ...
                $.request('create', request_data)
                    .then(function(){
                        ...
                        if($DATA.form_data === false){
                            ...
                            return $.when($.request('get_permissions'), $.request('get_form_data', request_data), $.request('get_list'));
                        }
                        return $.when($.request('get_permissions'), $.request('get_form_data', request_data))
                    })
                    .then(function(){

                        $this.pad('change', 'unit', '-1').data('current_permission', {id: id, year: year});
                        ...
                        if(is_first){
                            ...
                            $('#create-permission-menu-toggle-wrap').append(db.pad_toggle);

                            $this.pad('fix', false).pad('hide');

                            elements.select_license.val($DATA.defaults.license_prefix);
        ...
        show: function(){
            ...
            if(!this.hasClass('static')){
                $(document).on('click.pad', function(){
                    if($this.data('shown')){
                        if(!$this.data('not-hide') && $(event.target).closest(db.pad).length == 0 && $(event.target).closest('#ui-datepicker-div').length == 0 && $(event.target).closest('.ui-datepicker-prev').length == 0 && $(event.target).closest('.ui-datepicker-next').length == 0){
                            $this.pad('hide');
                        }
                    }
                });
    ...
    $.fn.pad = function(method, request){
        ...
        if(!this.is('#create-permission-pad-wrap')){
            $.error( 'Плагин jQuery.pad нужно применять на элемент #create-permission-pad-wrap' );
        }
...
(function($){
    ...
    var methods = {
        ...
        init: function(){
            ...
            this.data({
                ...
                elements: {
                    ...
                    items: function(year, id){
                        var items = $this.find('.permission-load');
                        if(id === undefined && year === undefined){
                            return $this.find('.permission-load');
            ...
            }).on('show.bs.collapse', '#permission-list .panel-collapse', function(){
            ...
            }).on('hide.bs.collapse', '#permission-list .panel-collapse', function(){
        ...
        load: function(id, year){
            ...
            function load(){
                ...
                var msg = consoleSentence('Загружено ОД: ' + ('[' + $DATA.units[$id].unit_name + ' ППР-' + $year + ']').emphasize('info'));

                form_elements.service.filter_button.find('option[value = "3"]').prop('selected', true);
                form_elements.service.view_button.removeClass('text-muted').prop('disabled', false);
                ...
                if($DATA.form_data.link === null){
                    form_elements.service.view_button.addClass('text-muted').attr('title', 'Сканированная версия отдельного разрешения не загружена').prop('disabled', true);
                } else {
                    form_elements.service.view_button1.add(form_elements.service.view_button2).data('link', $DATA.form_data.link);
                    form_elements.service.view_button.attr('title', 'Смотреть сканированную версию отдельного разрешения').prop('disabled', false);
                }

                db.form.data('current_permission', {id: $id, year: $year});
                $this.find('.permission-load.doc-active.current').removeClass('doc-active current');
    ...
    function refresh_docs_count(button){
        button.find('.docs-count').text($DATA.permissions.by_year.regular[button.attr('data-year')].length);
    }

    $.fn.list = function(method, request){

        if(!this.is('#permission-list-wrap')){
            $.error( 'Плагин jQuery.list нужно применять на элемент #permission-list-wrap' );
        }
    ...
    var methods = {
        ...
        init: function(){
            ...
            this.data({
                ...
                elements: {
                    service: {
                        ...
                        save_button: $('#permission-save'),
                        filter_button: $('#permission-filter'),
                        upload_button: $('#permission-upload'),
                        view_button: $('#permission-view'),
                        view_button1: $('#permission-view-1'),
                        view_button2: $('#permission-view-2'),
                        archive_button: $('#permission-archive')
                    },
                    header: $('#permission-form-header-wrap'),
                    data_container: function(){
                        return $this.find('.permission-input-group');
                    },
                    paragraphs_container: function(){
                        return $this.find('.nav-paragraph');
                    },
                    contents_container: function(){
                        return $this.find('.permission-input-group .tab-content');
                    },
                    date: function(){
                        return this.header.find('.permission-header-date');
                    },
                    license: function(){
                        return this.header.find('.permission-header-license')
                    }
                }
            });
            ...
            $(document).on('click.form.save', elements.service.save_button.selector, function(){
                ...
                output_confirm('Перенести в архив?', 'Данное отдельное разрешение будет перенесено в архив.</br>Подтвердите действие.')
                    .then(function(data){
                        if(data){
                            $this.form('archive');
                        }
            ...
            }).on('click.form.date', elements.date().selector + ', ' + elements.license().selector, function(){
                ...
                if(event.keyCode === 13){
                    ...
                    var current_permission = $this.data('current_permission');
                    $.request('save_date', {id: current_permission.id, year: current_permission.year, date: $(this).text().convertDate()}, {context: $(this)}).done(function(){
            ...
            }).on('keydown.form.license', elements.license().selector, function(){

                if(event.keyCode === 13){
                    event.preventDefault();
                    event.stopPropagation();
                    var current_permission = $this.data('current_permission');
                    $.request('save_license', {id: current_permission.id, year: current_permission.year, license: $(this).text()}, {context: $(this)}).done(function(){
            ...
            }).on('focusout.form.date', elements.date().selector + ', ' + elements.license().selector, function(){
                ...
                if (paragraph.length == 0){
                    ...
                    db.paragraphs('current', 'input').effect({effect: 'highlight', complete: function(){
                ...
                $.request('save_status',
                    {
                        ...
                        year: current_permission.year,
                        paragraph: paragraph,
                        status: status
                    }
                ).then(function(){
                        return $.request('get_stats', {id: current_permission.id, year: current_permission.year});
                    }).done(function(data){
        ...
        save: function(what){
            ...
            // ************************************** fields validation
                ...
                invalid_paragraphs = [];
                valid = true;
                // validate classificator fields
                msg = consoleSentence('Нельзя сохранить ОД: не установлен' + '[Классификатор]'.emphasize('danger'));
                db.paragraphs('input').each(function(i, val){
                    var name = $(this).val();
                    if (db.paragraphs('text', name, 'class').val() == "-1") {
                        valid = false;
                        invalid_paragraphs += name.consoleParagraph();
                    }

                });

                msg += consoleSentence('Пункты:' + invalid_paragraphs);

//                if (!valid) {
//                    cursor_release();
//                    db.form.form('release', 'form').form('release', 'service');
//                    db.console.console('log', msg, 0);
////                    info('Ошибка!'.emphasize('danger'), 'Нельзя сохранить данные. Подробнее: см. <a class = "output-anchor" href = "#permission-log">журнал</a>.'.emphasize('danger'));
//                    output_alert('Ошибка!', 'Нельзя сохранить данные. Подробнее: см. <a class = "output-anchor" href = "#permission-log">журнал</a>.');
//                    return false;
//                }

                // validate term fields for correctness
                valid = true;
                msg = consoleSentence('Нельзя сохранить ОД: неправильно установлен' + '[Срок]'.emphasize('danger'));
                db.paragraphs('input').each(function(i, val){
                    var name = $(this).val();
                ...
                msg += consoleSentence('Пункты:' + invalid_paragraphs) + consoleSentence('Формат даты:' + '[ ДД . ММ . ГГГГ ]'.emphasize('info'));
...
//                    info('Ошибка!'.emphasize('danger'), 'Нельзя сохранить данные. Подробнее: см. <a class = "output-anchor" href = "#permission-log">журнал</a>.'.emphasize('danger'));
                ...
                //validate section fields for availability
                valid = true;
                msg = consoleSentence('Нельзя сохранить ОД: неправильно установлен' + '[Раздел]'.emphasize('danger'));
                db.paragraphs('input').each(function(i, val){
                    var name = $(this).val();

                    if (db.paragraphs('text', name, 'section').val() == "-1") {
                        valid = false;
                        invalid_paragraphs += name.consoleParagraph();
                    }
...
//                return false;
                ...
                $.request('save', request_data).then(function(){
                    ...
                    if($DATA.save){
                        msg = 'Данные успешно сохранены. Отдельное разрешение' + ('[' + $DATA.units[current_permission.id].unit_name + ' ППР-' + current_permission.year + ']').emphasize('info');
                        db.console.console('stats', $DATA.stats).console('log', consoleSentence(msg), 1);
                        db.form.form('release', 'form').form('release', 'service');
        ...
        archive: function(){
            ...
            $.request('archive', request_data).then(function(){
                return $.request('get_permissions');
            }).then(function(){
                ...
                if($DATA.form_data === false){
                    ...
                    db.form.data('elements').header.html('<h3 class = "database-header user-select-none empty-data-message-header">Отдельное разрешение не загружено</h3>');
                    $.each(db.form.data('elements').service, function(i, val){
                    ...
                    release();
                    cursor_release();
                    db.console.console('log', 'Отдельное разрешение' + ('[' + unit_name + ' ППР-' + current_permission.year + ']').emphasize('info') + 'снято с учета', 1);
                    return false;
                }
                db.console.console('log', 'Отдельное разрешение' + ('[' + unit_name + ' ППР-' + current_permission.year + ']').emphasize('info') + 'снято с учета', 1);
                db.list.list('load');
    ...
    $.fn.form = function(method, request){
        ...
        if(!this.is('#permission-form-wrap')){
            $.error( 'Плагин jQuery.form нужно применять на элемент #permission-form-wrap' );
        }
    ...
    var methods = {
        init: function(){
            ...
            $(document).on('contextmenu', '.paragraph', function(){
                ...
                output_confirm('Удалить пункт?', 'Пункт будет удален и все данные полей этого пункта будут утеряны.</br>Вы действительно хотите удалить этот пункт?')
                    .then(function(data){
                        if(data){
        ...
        open: function(){
            ...
            reposition();
            db.paragraphmenu.data('triggered').addClass('paragraph-triggered');
            ...
            db.form.data('elements').paragraphs_container().on('scroll.paragraphmenu', function(){
                $this.paragraphmenu('close')
            });
            return this;
        },
        close: function(){
            ...
            this.hide();
            this.data('shown', false);
            this.data('triggered').removeClass('paragraph-triggered');
            $(document).off('click.paragraphmenu.click scroll.paragraphmenu');
            db.form.data('elements').paragraphs_container().off('scroll.paragraphmenu');
...
(function($){
    ...
    var methods = {
        ...
        edit: function(param1, param2){
            ...
            db.paragraphmenu.data('edit', param1);
            if(param1){
                db.paragraph.edited_val = input.val();
                input.focus();
                db.paragraphmenu.data('triggered').addClass('paragraph-edit');
            } else {
                $(document).off('focusout.paragraph.edit');
                if (param2 == 'all') {
                    this.removeClass('paragraph-edit');
                } else {
                    db.paragraphmenu.data('triggered').removeClass('paragraph-edit');
                }
            }
        ...
        save: function(){
            ...
            var current_permission = db.form.data('current_permission');
            var id = current_permission.id;
            var year = current_permission.year;
            var current_val = trim_paragraph(db.paragraph.edited_val);
            ...
            $.request('save_paragraph', request_data).then(function(){
                ...
                if($DATA.save_paragraph === true){
                    ...
                    tabpanel.find('select.ajax-data').each(function(){
                        temp[$(this).attr('name')] = $(this).val();
                    });

                    tabpanel.find('.ajax-data[name = "term"]').datepicker('destroy');
                    var new_li = li.clone();
                    var new_tabpanel = tabpanel.clone();

                    new_tabpanel.find('select.ajax-data').each(function(){
                        $(this).find(temp[$(this).attr('name')].wrapToAttr('value')).prop('selected', true);

                    });

                    new_tabpanel.find('.ajax-data[name = "term"]').datepicker(datepicker_options);
        ...
        remove: function(){
            ...
            var request_data = {
                'id': db.form.data('current_permission').id,
                'year': db.form.data('current_permission').year,
        ...
        menu: function(trigger){ // menu returns single elements not .paragraph collection
            ...
            db.paragraphmenu.paragraphmenu('close');
...
$('#permission-upload').on('click', function(){
    event.preventDefault();
    event.stopPropagation();
...
$('.upload-form').on('submit', function(){
    var current_permission = db.form.data('current_permission');
    $(this).attr('action', $(this).attr('action') + '&' + $.param({'id': current_permission.id, 'year': current_permission.year})) ;
});
...
$(document).on('click', '.output-anchor', function(){
    $('a[href = "#permission-log"]').tab('show');
    $('.modal.in').modal('hide');
});
...
$.when(
    ...
    .then(function(){
        ...
        console.log(range($DATA.defaults.max_year, $DATA.defaults.min_year));
        ...
        if($DATA.form_data === false){
            ...
            db.form.data('elements').header.append('<h3 class = "database-header user-select-none empty-data-message-header">Отдельное разрешение не загружено</h3>');
            $.each(db.form.data('elements').service, function(i, val){
                val.prop('disabled', true);
            });
...
//
//$.request('get_form_data', {id: '1', year: '2017'}).done(function(data){
//    console.log(data);
//    var data = data.form_data;
//    console.log(data);
//    $('.nav-paragraph').html(data.form_paragraphs);
//    $('.permission-input-group .tab-content').html(data.form_contents);
//});